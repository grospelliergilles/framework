...
//-----------------------------------------------------------------------------
/*---------------------------------------------------------------------------*/
/* BackendInterface.h                                          (C) 2026-2026 */
/*                                                                           */
 ...
 * \brief Backends implemented in Alina.
 *
 * A backend in Alina is a class that defines matrix and vector types together
...
/*!
 * \brief Metafunction that checks if two backends are compatible.
 *
...
/*---------------------------------------------------------------------------*/
/*!
 * \brief Implementation for function returning the number of rows in a matrix.
 *
 * \note Used in rows().
 */
...
/*---------------------------------------------------------------------------*/
/*!
 * \brief Implementation for function returning the number of columns in a matrix.
 * \note Used in cols()
 */
template <class Matrix, class Enable = void>
struct cols_impl
...
/*---------------------------------------------------------------------------*/
/*!
 * \brief Implementation for function returning number of bytes allocated for a matrix/vector.
 *
 * \note Used in bytes()
 */
template <class T, class Enable = void>
...
/*---------------------------------------------------------------------------*/
/*!
 * \brief Implementation for function returning the number of nonzeros in a matrix.
 *
...
/*---------------------------------------------------------------------------*/
/*!
 * \brief Implementation for function returning the number of nonzeros in a matrix row.
 *
...
/*---------------------------------------------------------------------------*/
/*!
 * \brief Metafunction returning the row iterator type for a matrix type.
 *
...
/*---------------------------------------------------------------------------*/
/*!
 * \brief Implementation for function returning row iterator for a matrix.
 *
 * \note This only has to be implemented in the backend if support for serial
 * smoothers (Gauss-Seidel or ILU0) is required.
...
/*---------------------------------------------------------------------------*/
/*!
 * \brief Implementation for matrix-vector product.
 *
 * \note Used in spmv()
 */
...
/*---------------------------------------------------------------------------*/
/*!
 * \brief Implementation for residual error compuatation.
 *
...
/*!
 * \brief Implementation for zeroing out a vector.
 *
 * \note Used in clear().
 */
...
/*!
 * \brief Implementation for vector copy.
 *
 * \note Used in copy().
 */
...
/*!
 * \brief Implementation for inner product.
 *
...
/*!
 * \brief Implementation for linear combination of two vectors.
 *
 * \note Used in axpby().
 */
...
/*!
 * \brief Implementation for linear combination of three vectors.
 *
...
/*!
 * \brief Reinterpret the vector to be compatible with the matrix value type.
 */
...
template <class Vector1, class Vector2>
typename math::inner_product_impl<typename value_type<Vector1>::type>::return_type
...
/*---------------------------------------------------------------------------*/
/*!
 * \brief Computes linear combination of two vectors.
 * \f[y = ax + by.\f]
 */
...
/*---------------------------------------------------------------------------*/
/*!
 * \brief Computes linear combination of three vectors.
 *
 * \f[z = ax + by + cz.\f]
 */
...
typename reinterpret_as_rhs_impl<MatrixValue,
                                 typename std::decay<Vector>::type,
                                 std::is_const<typename std::remove_reference<Vector>::type>::value>::return_type
reinterpret_as_rhs(Vector&& x)
{
  return reinterpret_as_rhs_impl<MatrixValue,
                                 typename std::decay<Vector>::type,
                                 std::is_const<typename std::remove_reference<Vector>::type>::value>::get(std::forward<Vector>(x));
}
