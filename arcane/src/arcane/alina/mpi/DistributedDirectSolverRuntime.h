...
class DistributedDirectSolverRuntime
{
 public:
  ...
  DistributedDirectSolverRuntime(Alina::mpi::mpi_communicator comm, const Matrix& A, params prm = params())
 ...
 private:
  ...
  do_construct(Alina::mpi::mpi_communicator comm, const Matrix& A, const params& prm)
  {
    handle = static_cast<void*>(new S(comm, A, prm));
  }
  ...
  template <class S, class V, class Matrix>
  typename std::enable_if<!std::is_same<V, float>::value && !std::is_same<V, double>::value, void>::type
  do_construct(Alina::mpi::mpi_communicator, const Matrix&, const params&)
  {
